...
const VideoCard = ({ info }) => {
  ...
  return (
    <div className='p-2 m-2 w-72 shadow-lg rounded-lg cursor-pointer hover:scale-105 transition-transform duration-200'>
      <img 
        className='rounded-lg w-full h-40 object-cover' // Added object-cover for consistent image sizing
        src={thumbnails.medium.url} 
        alt="thumbnail" 
      />
      <ul className='mt-2'>
        <li className='font-bold text-base line-clamp-2'>{title}</li> {/* line-clamp-2 for multi-line title */}
